...
library("SASxport")
library(dplyr)
data_11 <-read.xport("11data.XPT")
data <- data_11 %>% mutate(fips=X.STATE * 1000 + CTYCODE1) %>%
  filter(!is.na(fips)) %>%
  select(fips, X.STATE, CTYCODE1, IYEAR,MENTHLTH, CVDINFR4, CVDCRHD4,CVDSTRK3, ASTHMA3, AGE, SEX, ORACE2, EDUCA, INCOME2, MARITAL, EMPLOY, HLTHPLN1, EXERANY2, X.RFSMOK3,DRNKANY5, X.BMI5CAT, X.BMI5, GENHLTH, QLACTLM2)

data1 <- data_11 %>% mutate(state=X.STATE) %>%
  filter(!is.na(state)) %>%
  select(state, IYEAR,MENTHLTH, CVDINFR4, CVDCRHD4,CVDSTRK3, ASTHMA3, AGE, SEX, ORACE2, EDUCA, INCOME2, MARITAL, EMPLOY, HLTHPLN1, EXERANY2, X.RFSMOK3,DRNKANY5, X.BMI5CAT, X.BMI5, GENHLTH, QLACTLM2)
```

NA
```{r}
data$CTYCODE1[data$CTYCODE1 %in% c(777,999)] <- c(NA)
...
data$ASTHMA3[data$ASTHMA3 %in% c(7,9)] <- c(NA) 
...
data$HLTHPLN1[data$HLTHPLN1 %in%  c(7,9)] <- c(NA) 
data$EXERANY2[data$EXERANY2 %in% c(7,9)] <- c(NA) 
data$X.RFSMOK3[data$X.RFSMOK3 == 9] <- c(NA) 
data$DRNKANY5[data$DRNKANY5 %in% c(7,9)] <- c(NA) 
data$X.BMI5CAT[data$X.BMI5CAT == 9] <- c(NA) 
...
data1$ASTHMA3[data1$ASTHMA3 %in% c(7,9)] <- c(NA) 
...
data1$HLTHPLN1[data1$HLTHPLN1 %in%  c(7,9)] <- c(NA) 
data1$EXERANY2[data1$EXERANY2 %in% c(7,9)] <- c(NA) 
data1$X.RFSMOK3[data1$X.RFSMOK3 == 9] <- c(NA) 
data1$DRNKANY5[data1$DRNKANY5 %in% c(7,9)] <- c(NA) 
data1$X.BMI5CAT[data1$X.BMI5CAT == 9] <- c(NA) 
...
SELECT
```{r}
data11 <-data %>% group_by(fips) %>%
         summarise(state = mean(X.STATE, na.rm = TRUE),
         county = mean(CTYCODE1, na.rm = TRUE),
         year = 2011,
         ...
         asthma = mean(ASTHMA3 == 1, na.rm = TRUE),
         ...
         HLTHPLN1 = mean(HLTHPLN1 == 1, na.rm = TRUE),
         exercise = mean(EXERANY2 == 1, na.rm = TRUE),
         smoke = mean(X.RFSMOK3 == 2, na.rm = TRUE),
         drink = mean(DRNKANY5 == 1, na.rm = TRUE),
         bmi_cat1 = mean(X.BMI5CAT == 1 | X.BMI5CAT == 2, na.rm =TRUE),
         bmi_cat2 = mean(X.BMI5CAT == 3, na.rm = TRUE),
         bmi_cat3 = mean(X.BMI5CAT == 4, na.rm = TRUE),
         bmi_cts = mean(X.BMI5/100,na.rm = TRUE),
...
write.csv(data11, file = "2011_data.csv", row.names = FALSE)
```

state level
```{r}
data111 <-data1 %>% group_by(state) %>%
         summarise(year = 2011,
         menthlth = mean(MENTHLTH, na.rm= TRUE),
         heartattack = mean(CVDINFR4 == 1, na.rm = TRUE),
         ACheartdis = mean(CVDCRHD4 == 1, na.rm = TRUE),
         ...
         genhlth_po = mean(GENHLTH == 5, na.rm = TRUE),
         qlactlm = mean(QLACTLM2 == 1, na.rm = TRUE))
write.csv(data111, file = "2011_data_state.csv", row.names = FALSE)
```