...
This Rmd file is for data cleaning and aggregating BRFSS data into county level for the year 2001
...
```{r}
dat_col_01 = data_01 %>%
  select(X.STATE,CTYCODE,IMONTH,MENTHLTH,CVDINFR2,CVDCRHD2,CVDSTRK2,ASTHMA2,AGE,SEX,ORACE2,EDUCA,INCOME2,MARITAL,EMPLOY,HLTHPLAN,EXERANY2,X.RFSMOK2,X.BMI2CAT,X.BMI2,GENHLTH,QLACTLM2)

```

create fips as key and create year variable:
```{r}
dat_col_01 = dat_col_01 %>%
...
2.hearattack
```{r}
count_fips = dat_col_01 %>%
  select(fips,CVDINFR2) %>%
  filter(!is.na(CVDINFR2) & CVDINFR2 != 7 & CVDINFR2 != 9) %>%
  group_by(fips) %>%
  dplyr::summarise(countF = n())
  
count_h = dat_col_01 %>%
  group_by(fips,CVDINFR2) %>%
  dplyr::summarise(n = n())

tab_heart = count_h %>% 
  left_join(count_fips,by = c('fips' = 'fips')) %>%
  mutate(hearattack = n/countF) %>%
  filter(!is.na(CVDINFR2) & CVDINFR2 == 1) %>%
...
```{r}
count_fips = dat_col_01 %>%
  select(fips,CVDCRHD2) %>%
  filter(!is.na(CVDCRHD2) & CVDCRHD2 !=7 & CVDCRHD2 != 9) %>%
  group_by(fips) %>%
  dplyr::summarise(countF = n())
  
count_a = dat_col_01 %>%
  group_by(fips,CVDCRHD2) %>%
  dplyr::summarise(n = n())
...
tab_ac = count_a %>% 
  ...
  filter(!is.na(CVDCRHD2) & CVDCRHD2 == 1) %>%
...
```{r}
count_fips = dat_col_01 %>%
  select(fips,CVDSTRK2) %>%
  filter(!is.na(CVDSTRK2) & CVDSTRK2 != 7 & CVDSTRK2 != 9) %>%
  group_by(fips) %>%
  dplyr::summarise(countF = n())
  
count_s = dat_col_01 %>%
  group_by(fips,CVDSTRK2) %>%
  dplyr::summarise(n = n())
...
tab_st = count_s %>% 
  ...
  filter(!is.na(CVDSTRK2) & CVDSTRK2 == 1) %>%
  select(fips,stroke)
```
...
18.bmi_cts
```{r}
tab_bmicts = dat_col_01 %>%
  select(fips,X.BMI2) %>%
  filter(X.BMI2 != 999999) %>%
  group_by(fips) %>%
...
#var38
tab_drk = data.frame(rep("NA",nrow(final_00)))
final_00$rep..NA...nrow.final_00.. = NA
colnames(final_00)[38] = "drink"
...
save as csv:
```{r,eval= FALSE}
write.csv(final_00,file = "2001_data.csv",row.names = F)
```

more steps are needed to finalize the model since I didn't consider the presense of "777" and "999" in the CTYCODE
```{r,eval=FALSE}
data_2001 = read_csv("2001_data.csv")
data_2001 = data_2001 %>%
  filter(county != 777 & county != 999) %>%
  mutate(bmi_cts = bmi_cts/10000) %>%
  select(-metro)

data_2001 = unique(data_2001)
write_csv(data_2001,"data_2001.csv")
```