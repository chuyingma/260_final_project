...
library("SASxport")
library(dplyr)
data_12 <-read.xport("12data.XPT")
data <- data_12 %>% mutate(fips=X.STATE * 1000 + CTYCODE1) %>%
  filter(!is.na(fips)) %>%
...
data1 <- data_12 %>% mutate(state=X.STATE) %>%
  filter(!is.na(state)) %>%
...
write.csv(data12, file = "2012_data.csv")
```

state level
```{r}
data121 <-data1 %>% group_by(state) %>%
         summarise(year = 2012,
         menthlth = mean(MENTHLTH, na.rm= TRUE),
         heartattack = mean(CVDINFR4 == 1, na.rm = TRUE),
         ACheartdis = mean(CVDCRHD4 == 1, na.rm = TRUE),
         ...
         genhlth_po = mean(GENHLTH == 5, na.rm = TRUE),
         qlactlm = mean(QLACTLM2 == 1, na.rm = TRUE))
write.csv(data121, file = "2012_data_state.csv")
```