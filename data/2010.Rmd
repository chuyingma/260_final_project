...
This Rmd file is for data cleaning and aggregating BRFSS data into county level and state level for the year 2010
```{r}
library("SASxport")
library(dplyr)
data_10<-read.xport("10data.XPT")
head(data_10)
data <- data_10 %>% mutate(fips=X.STATE * 1000 + CTYCODE) %>%
  filter(!is.na(fips)) %>%
...
data1 <- data_10 %>% mutate(state=X.STATE) %>%
  filter(!is.na(state)) %>%
...
data$HLTHPLAN[data$HLTHPLAN == 9] <- c(NA) 
...
#write.csv(data, file = "2010_data_raw.csv", row.names = FALSE)
```
...
data1$HLTHPLAN[data1$HLTHPLAN == 9] <- c(NA) 
...
#write.csv(data10, file = "2010_data.csv", row.names = FALSE)
```

state level
```{r}
data101 <-data1 %>% group_by(state) %>%
         summarise(year = 2010,
         menthlth = mean(MENTHLTH, na.rm= TRUE),
         heartattack = mean(CVDINFR4 == 1, na.rm = TRUE),
         ACheartdis = mean(CVDCRHD4 == 1, na.rm = TRUE),
         ...
         genhlth_po = mean(GENHLTH == 5, na.rm = TRUE),
         qlactlm = mean(QLACTLM2 == 1, na.rm = TRUE))
#write.csv(data101, file = "2010_data_state.csv", row.names = FALSE)
```