...
This Rmd file is for data cleaning and aggregating BRFSS data into county level for the year 2000
...
```{r}
data_00 = read.xport("~/Desktop/Harvard/fall 2017/bst260/project/CDBRFS00.XPT")
names(data_00)
```
...
create fips as key and create year variable:
```{r}
dat_col_00 = dat_col_00 %>%
  filter(!is.na(CTYCODE) & !is.na(X.STATE)) %>%
  mutate(fips = 1000* as.numeric(X.STATE) + as.numeric(CTYCODE)) %>%
  mutate(year = 2000) %>%
  select(fips, X.STATE,CTYCODE,year,IMONTH,MENTHLTH,CVDINFAR,CVDCORHD,CVDSTROK,ASTHMA,AGE,SEX,ORACE,EDUCA,INCOME2,MARITAL,EMPLOY,HLTHPLAN,EXERANY,X.RFSMOK2,DRINKANY,X.BMI2CAT,X.BMI2,GENHLTH,QLACTLMT)
...
menthlth = dat_col_00 %>%
  ...
  filter(MENTHLTH != 77 & MENTHLTH != 88 & MENTHLTH != 99) %>%
  group_by(fips) %>%
...
2.hearattack
```{r}
count_fips = dat_col_00 %>%
  select(fips,CVDINFAR) %>%
  filter(!is.na(CVDINFAR) & CVDINFAR != 7 & CVDINFAR != 9) %>%
  group_by(fips) %>%
  dplyr::summarise(countF = n())
  
count_h = dat_col_00 %>%
  group_by(fips,CVDINFAR) %>%
  dplyr::summarise(n = n())

tab_heart = count_h %>% 
  left_join(count_fips,by = c('fips' = 'fips')) %>%
  mutate(hearattack = n/countF) %>%
  filter(!is.na(CVDINFAR) & CVDINFAR == 1) %>%
...
```{r}
count_fips = dat_col_00 %>%
  select(fips,CVDCORHD) %>%
  filter(!is.na(CVDCORHD) & CVDCORHD !=7 & CVDCORHD != 9) %>%
  group_by(fips) %>%
  dplyr::summarise(countF = n())
  
count_a = dat_col_00 %>%
  group_by(fips,CVDCORHD) %>%
  dplyr::summarise(n = n())
...
tab_ac = count_a %>% 
  ...
  filter(!is.na(CVDCORHD) & CVDCORHD == 1) %>%
...
```{r}
count_fips = dat_col_00 %>%
  select(fips,CVDSTROK) %>%
  filter(!is.na(CVDSTROK) & CVDSTROK != 7 & CVDSTROK != 9) %>%
  group_by(fips) %>%
  dplyr::summarise(countF = n())
  
count_s = dat_col_00 %>%
  group_by(fips,CVDSTROK) %>%
  dplyr::summarise(n = n())
...
tab_st = count_s %>% 
  ...
  filter(!is.na(CVDSTROK) & CVDSTROK == 1) %>%
  select(fips,stroke)
```
...
count_fips = dat_col_00 %>%
  select(fips,ASTHMA) %>%
  filter(!is.na(ASTHMA) & ASTHMA != 7 & ASTHMA != 9) %>%
  group_by(fips) %>%
  dplyr::summarise(countF = n())
  
count_as = dat_col_00 %>%
  group_by(fips,ASTHMA) %>%
  dplyr::summarise(n = n())

tab_as = count_as %>% 
  left_join(count_fips,by = c('fips' = 'fips')) %>%
  mutate(asthma = n/countF) %>%
  filter(!is.na(ASTHMA) & ASTHMA == 1) %>%
  select(fips,asthma)
```
...
count_fips = dat_col_00 %>%
  select(fips,ORACE) %>%
  filter(!is.na(ORACE) & ORACE != 7 & ORACE != 8 & ORACE != 9) %>%
  group_by(fips) %>%
  dplyr::summarise(countF = n())
  
count_race = dat_col_00 %>%
  group_by(fips,ORACE) %>%
  dplyr::summarise(n = n())
...
white = count_race %>% 
  ...
  filter(!is.na(ORACE) & ORACE == 1) %>%
  select(fips,white)

black = count_race %>% 
  left_join(count_fips,by = c('fips' = 'fips')) %>%
  mutate(black = n/countF) %>%
  filter(!is.na(ORACE) & ORACE == 2) %>%
  select(fips,black)

asian = count_race %>% 
  left_join(count_fips,by = c('fips' = 'fips')) %>%
  mutate(asian = n/countF) %>%
  filter(!is.na(ORACE) & ORACE == 3) %>%
  select(fips,asian)

other_race = count_race %>% 
  left_join(count_fips,by = c('fips' = 'fips')) %>%
  mutate(race1 = n/countF) %>%
  filter(!is.na(ORACE) & ORACE %in% c(4,5,6)) %>%
  group_by(fips) %>%
...
count_fips = dat_col_00 %>%
  select(fips,EXERANY) %>%
  filter(!is.na(EXERANY) & EXERANY != 7 & EXERANY != 9) %>%
  group_by(fips) %>%
  dplyr::summarise(countF = n())
  
count_ex = dat_col_00 %>%
  group_by(fips,EXERANY) %>%
  dplyr::summarise(n = n())

tab_ex = count_ex %>% 
  left_join(count_fips,by = c('fips' = 'fips')) %>%
  mutate(exercise = n/countF) %>%
  filter(!is.na(EXERANY) & EXERANY == 1) %>%
...
```{r}
count_fips = dat_col_00 %>%
  select(fips,X.RFSMOK2) %>%
  filter(!is.na(X.RFSMOK2) & X.RFSMOK2 != 9) %>%
  group_by(fips) %>%
  dplyr::summarise(countF = n())
  
count_sm = dat_col_00 %>%
  group_by(fips,X.RFSMOK2) %>%
  dplyr::summarise(n = n())

tab_sm = count_sm %>% 
  left_join(count_fips,by = c('fips' = 'fips')) %>%
  mutate(smoke = n/countF) %>%
  filter(!is.na(X.RFSMOK2) & X.RFSMOK2 == 2) %>%
  select(fips,smoke)
```
...
```{r}
count_fips = dat_col_00 %>%
  select(fips,DRINKANY) %>%
  filter(!is.na(DRINKANY) & DRINKANY != 7 & DRINKANY != 9) %>%
  group_by(fips) %>%
  dplyr::summarise(countF = n())
  
count_drk = dat_col_00 %>%
  group_by(fips,DRINKANY) %>%
  dplyr::summarise(n = n())

tab_drk = count_drk %>% 
  left_join(count_fips,by = c('fips' = 'fips')) %>%
  mutate(drink = n/countF) %>%
  filter(!is.na(DRINKANY) & DRINKANY == 1) %>%
  select(fips,drink)
```
...
```{r}
count_fips = dat_col_00 %>%
  select(fips,X.BMI2CAT) %>%
  filter(!is.na(X.BMI2CAT) & X.BMI2CAT != 9) %>%
  group_by(fips) %>%
  dplyr::summarise(countF = n())
  
count_bmicat = dat_col_00 %>%
  group_by(fips,X.BMI2CAT) %>%
  dplyr::summarise(n = n())

tab_bmicat1 = count_bmicat %>% 
  left_join(count_fips,by = c('fips' = 'fips')) %>%
  mutate(bmi_cat1 = n/countF) %>%
  filter(!is.na(X.BMI2CAT) & X.BMI2CAT == 1) %>%
  select(fips,bmi_cat1)

tab_bmicat2 = count_bmicat %>% 
  left_join(count_fips,by = c('fips' = 'fips')) %>%
  mutate(bmi_cat2 = n/countF) %>%
  filter(!is.na(X.BMI2CAT) & X.BMI2CAT == 2) %>%
  select(fips,bmi_cat2)

tab_bmicat3 = count_bmicat %>% 
  left_join(count_fips,by = c('fips' = 'fips')) %>%
  mutate(bmi_cat3 = n/countF) %>%
  filter(!is.na(X.BMI2CAT) & X.BMI2CAT == 3) %>%
  select(fips,bmi_cat3)
```

18.bmi_cts
```{r}
tab_bmicts = dat_col_00 %>%
  select(fips,X.BMI2) %>%
  filter(X.BMI2 != 999) %>%
  group_by(fips) %>%
  dplyr::summarise(bmi_cts = mean(X.BMI2))
```
...
count_fips = dat_col_00 %>%
  select(fips,GENHLTH) %>%
  filter(!is.na(GENHLTH) & GENHLTH != 9) %>%
  group_by(fips) %>%
  dplyr::summarise(countF = n())
  
count_genh = dat_col_00 %>%
...
```{r}
count_fips = dat_col_00 %>%
  select(fips,QLACTLMT) %>%
  filter(!is.na(QLACTLMT) & QLACTLMT != 7 & QLACTLMT != 9) %>%
  group_by(fips) %>%
  dplyr::summarise(countF = n())
  
count_ql = dat_col_00 %>%
  group_by(fips,QLACTLMT) %>%
  dplyr::summarise(n = n())

tab_ql = count_ql %>% 
  left_join(count_fips,by = c('fips' = 'fips')) %>%
  mutate(qlactlm = n/countF) %>%
  filter(!is.na(QLACTLMT) & QLACTLMT == 1) %>%
  select(fips,qlactlm)
```

left join all sub-tables:
```{r,eval=FALSE}
tab_first = dat_col_00 %>%
  mutate(metro = NA) %>%
  select(fips,state = X.STATE,county = CTYCODE, metro,year)
...
save as csv:
```{r,eval=FALSE}
write.csv(final_00,file = "2000_data.csv",row.names = F)
```

Actually I didn't consider the presence of "777" and "999" in this dataset, 
so more steps are needed to finalize the dataset:
```{r,eval=FALSE}
data_2000 = read_csv("2000_data.csv")
data_2000 = data_2000 %>%
  filter(county != 777 & county != 999) %>%
  mutate(bmi_cts = bmi_cts/10) %>%
  select(-metro)

data_2000 = unique(data_2000)
write_csv(data_2000,"data_2000.csv")
```
