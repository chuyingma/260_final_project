...
This Rmd file is for data cleaning and aggregating BRFSS data into county level for the year 2005
...
try to aggregate for the data in 2005
```{r}
...
```{r}
dat_col_05 = data_05 %>%
  select(X.STATE,CTYCODE,MSCODE,IMONTH,MENTHLTH,CVDINFR3,CVDCRHD3,CVDSTRK3,ASTHMA2,AGE,SEX,ORACE2,EDUCA,INCOME2,MARITAL,EMPLOY,HLTHPLAN,EXERANY2,X.RFSMOK3,DRNKANY4,X.BMI4CAT,X.BMI4,GENHLTH,QLACTLM2)
...
2.hearattack
```{r}
count_fips = dat_col_05 %>%
  select(fips,CVDINFR3) %>%
  filter(!is.na(CVDINFR3) & CVDINFR3 != 7 & CVDINFR3 != 9) %>%
  group_by(fips) %>%
  dplyr::summarise(countF = n())
  
count_h = dat_col_05 %>%
  group_by(fips,CVDINFR3) %>%
  dplyr::summarise(n = n())

tab_heart = count_h %>% 
  left_join(count_fips,by = c('fips' = 'fips')) %>%
  mutate(hearattack = n/countF) %>%
  filter(!is.na(CVDINFR3) & CVDINFR3 == 1) %>%
...
```{r}
count_fips = dat_col_05 %>%
  select(fips,CVDCRHD3) %>%
  filter(!is.na(CVDCRHD3) & CVDCRHD3 !=7 & CVDCRHD3 != 9) %>%
  group_by(fips) %>%
  dplyr::summarise(countF = n())
  
count_a = dat_col_05 %>%
  group_by(fips,CVDCRHD3) %>%
  dplyr::summarise(n = n())
...
tab_ac = count_a %>% 
  ...
  filter(!is.na(CVDCRHD3) & CVDCRHD3 == 1) %>%
...
save as csv:
```{r,eval=FALSE}
write.csv(final_00,file = "2005_data.csv",row.names = F)
```

more steps are needed to finalize the model since I didn't consider the presense of "777" and "999" in the CTYCODE
```{r,eval=FALSE}
data_2005 = read_csv("2005_data.csv")
data_2005 = data_2005 %>%
  filter(county != 777 & county != 999) %>%
  mutate(bmi_cts = bmi_cts/100) %>%
  select(-metro)

data_2005 = unique(data_2005)

write_csv(data_2005,"~/Desktop/Harvard/fall 2017/bst260/project/data_2005.csv")
```