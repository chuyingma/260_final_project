...
This Rmd file is for data cleaning and aggregating BRFSS data into county level for the year 2006
...
save as csv:
```{r,eval=FALSE}
write.csv(final_00,file = "2006_data.csv",row.names = F)
```

more steps are needed to finalize the model since I didn't consider the presense of "777" and "999" in the CTYCODE
```{r,eval=FALSE}
data_2006 = read_csv("2006_data.csv")
data_2006 = data_2006 %>%
  filter(county != 777 & county != 999) %>%
  mutate(bmi_cts = bmi_cts/100) %>%
  select(-metro)

data_2006 = unique(data_2006)

write_csv(data_2006,"~/Desktop/Harvard/fall 2017/bst260/project/data_2006.csv")
```