...
This Rmd file is for data cleaning and aggregating BRFSS data into county level for the year 2004
...
```{r}
dat_col_04 = data_04 %>%
  select(X.STATE,CTYCODE,MENTHLTH,CVDINFR2,CVDCRHD2,CVDSTRK2,ASTHMA2,AGE,SEX,ORACE2,EDUCA,INCOME2,MARITAL,EMPLOY,HLTHPLAN,EXERANY2,X.RFSMOK2,DRNKANY3,X.BMI4CAT,X.BMI4,GENHLTH,QLACTLM2)
#no MSCODE in 2004 
```

create fips and create year variable:
```{r}
dat_col_04 = dat_col_04 %>%
...
tab_inc1 = count_inc %>% 
  ...
  filter(!is.na(INCOME2) & INCOME2 == 1) %>%
  select(fips,income1)

tab_inc2 = count_inc %>% 
  left_join(count_fips,by = c('fips' = 'fips')) %>%
  mutate(income2 = n/countF) %>%
  filter(!is.na(INCOME2) & INCOME2 == 2) %>%
  select(fips,income2)

tab_inc3 = count_inc %>% 
  left_join(count_fips,by = c('fips' = 'fips')) %>%
  mutate(income3 = n/countF) %>%
  filter(!is.na(INCOME2) & INCOME2 == 3) %>%
  select(fips,income3)

tab_inc4 = count_inc %>% 
  left_join(count_fips,by = c('fips' = 'fips')) %>%
  mutate(income4 = n/countF) %>%
  filter(!is.na(INCOME2) & INCOME2 == 4) %>%
  select(fips,income4)

tab_inc5 = count_inc %>% 
  left_join(count_fips,by = c('fips' = 'fips')) %>%
  mutate(income5 = n/countF) %>%
  filter(!is.na(INCOME2) & INCOME2 == 5) %>%
  select(fips,income5)

tab_inc6 = count_inc %>% 
  left_join(count_fips,by = c('fips' = 'fips')) %>%
  mutate(income6 = n/countF) %>%
  filter(!is.na(INCOME2) & INCOME2 == 6) %>%
  select(fips,income6)

tab_inc7 = count_inc %>% 
  left_join(count_fips,by = c('fips' = 'fips')) %>%
  mutate(income7 = n/countF) %>%
  filter(!is.na(INCOME2) & INCOME2 == 7) %>%
  select(fips,income7)

tab_inc8 = count_inc %>% 
  left_join(count_fips,by = c('fips' = 'fips')) %>%
  mutate(income8 = n/countF) %>%
  filter(!is.na(INCOME2) & INCOME2 == 8) %>%
  select(fips,income8)
```
...
```{r}
count_fips = dat_col_04 %>%
  select(fips,DRNKANY3) %>%
  filter(!is.na(DRNKANY3) & DRNKANY3 != 7 & DRNKANY3 != 9) %>%
  group_by(fips) %>%
  dplyr::summarise(countF = n())
  
count_drk = dat_col_04 %>%
  group_by(fips,DRNKANY3) %>%
  dplyr::summarise(n = n())

tab_drk = count_drk %>% 
  left_join(count_fips,by = c('fips' = 'fips')) %>%
  mutate(drink = n/countF) %>%
  filter(!is.na(DRNKANY3) & DRNKANY3 == 1) %>%
  select(fips,drink)
```
...
save as csv:
```{r,eval=FALSE}
write.csv(final_00,file = "2004_data.csv",row.names = F)
```

more steps are needed to finalize the model since I didn't consider the presense of "777" and "999" in the CTYCODE:
```{r,eval=FALSE}
data_2004 = read_csv("2004_data.csv")
data_2004 = data_2004 %>%
  filter(county != 777 & county != 999) %>%
  mutate(bmi_cts = bmi_cts/100) %>%
  select(-metro)

data_2004 = unique(data_2004)
getwd()
write_csv(data_2004,"~/Desktop/Harvard/fall 2017/bst260/project/data_2004.csv")
```