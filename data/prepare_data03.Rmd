...
This Rmd file is for data cleaning and aggregating BRFSS data into county level for the year 2003
...
```{r}
dat_col_03 = data_03 %>%
  select(X.STATE,CTYCODE,IMONTH,MENTHLTH,CVDINFR2,CVDCRHD2,CVDSTRK2,ASTHMA2,AGE,SEX,ORACE2,EDUCA,INCOME2,MARITAL,EMPLOY,HLTHPLAN,EXERANY2,X.RFSMOK2,DRNKANY3,X.BMI3CAT,X.BMI3,GENHLTH,QLACTLM2)

```

create fips and create year variable:
```{r}
dat_col_03 = dat_col_03 %>%
...
```{r}
count_fips = dat_col_03 %>%
  select(fips,X.BMI3CAT) %>%
  filter(!is.na(X.BMI3CAT) & X.BMI3CAT != 9) %>%
  group_by(fips) %>%
  dplyr::summarise(countF = n())
  
count_bmicat = dat_col_03 %>%
  group_by(fips,X.BMI3CAT) %>%
  dplyr::summarise(n = n())

tab_bmicat1 = count_bmicat %>% 
  left_join(count_fips,by = c('fips' = 'fips')) %>%
  mutate(bmi_cat1 = n/countF) %>%
  filter(!is.na(X.BMI3CAT) & X.BMI3CAT == 1) %>%
  select(fips,bmi_cat1)

tab_bmicat2 = count_bmicat %>% 
  left_join(count_fips,by = c('fips' = 'fips')) %>%
  mutate(bmi_cat2 = n/countF) %>%
  filter(!is.na(X.BMI3CAT) & X.BMI3CAT == 2) %>%
  select(fips,bmi_cat2)

tab_bmicat3 = count_bmicat %>% 
  left_join(count_fips,by = c('fips' = 'fips')) %>%
  mutate(bmi_cat3 = n/countF) %>%
  filter(!is.na(X.BMI3CAT) & X.BMI3CAT == 3) %>%
  select(fips,bmi_cat3)
```

18.bmi_cts
```{r}
tab_bmicts = dat_col_03 %>%
  select(fips,X.BMI3) %>%
  filter(X.BMI3 != 9999) %>%
  group_by(fips) %>%
  dplyr::summarise(bmi_cts = mean(X.BMI3))
```
