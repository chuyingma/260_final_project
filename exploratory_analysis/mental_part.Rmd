---
title: "Visualization -- Part 1. Mental Health"
author: "Chuying Ma"
date: "11/26/2017"
...
This Rmd file is for studying the relationship between other variables including environmental exposures, socioeconomic variables and mental health.

But it also involve some related data cleaning steps for better exploration.
...
Map from different years in the same plot using facet:
...
###Part 2. write state level data

Write state level data for exposure, which is for exploring the relationship between environmental exposures and mental health:

combine with pm2.5 and ozone, and save those in csv files for different years:
```{r,eval=FALSE}
...
run linear regression for predicting mental health condition with all covariates

backward selection are used here
...
###Part 4. Map for state level data

aggregate data into state level:
```{r}
st.codes<-data.frame(
...
SELECT variables from raw data
```{r,eval=FALSE}
...
##Conclusion:
Individuals answered the same question that "Now thinking about your mental health, which includes stress, depression, and problems with emotions, for how many days during the past 30 days was your mental health not good? " for any BRFSS survey year. Can you imagine which counties or states are the ones with worst average self-reported mental health condition?
...
Mental health condition and pm 2.5, ozone are positively correlated



###Part 6. Write full dataset with all 3 environmental exposures
...
###Part 7. check the colinearity via correlation map
...
##Conclusion
Considering the possibility of colinearity, we also checked the correlation map to see if there are possible pairs that have high correlation. It turns out that it is fine and we don't need worry about it for our linear model!


###Part 8. Plot fitted values for linear regression vs actual values
